...
$(document).ready(function () {
    ...
    switch (buttonOption) {
      ...
      case 'Mercado Pago': // Método de pago de Mercado Pago
        ...
        modal
          .find('.modal-body')
          .html(
            "<center><div id='qr'></div><div id='countDown'></div><br/><div id='loading'></div><br/><div id='orderStatus'></div><div id='orderResponse'></div></center>"
          )
        ...
        $.get('api/pos/get/', { external_id: external_id }, function (data) {
          ...
          if (data.paging.total > 0) {
            ...
            $('#qr').html(
              "<img with='350px' height='350px' src='" +
                data.results[0].qr.image +
                "'>"
            )
            ...
            $.post(
              'api/order/create/',
              { external_id: external_id, json: JSON.stringify(orderJSON) },
              function (data) {
            ...
            $('#qr').html(
              "This External POS ID: 'EPOSID' does not exists<br/>for this Store ID: 'Store_id'"
                .replace('EPOSID', external_id)
  ...
  function showProductList() {
    ...
    for (i in items) {
      $('#productList').append(
        "<tr><th scope='row'>" +
          items[i].id +
          "</th><td><img with='60px' height='60px' src='" +
          items[i].picture_url +
          "'></td><td>" +
          items[i].title +
          '</td><td>' +
    ...
    $('#productList').append(
      "<tr><th scope='row'></th><td></td><td></td><td></td><td><b>Total:</b></td><td><b>" +
        total +
        '</b></td></tr>'
    )
  } // end showProductList
  ...
  $('#states').change(function () {
    ...
    $.get('https://api.mercadolibre.com/states/' + selectedState, {}, function (
      cities
    ) {
  ...
  $('#createStore').click(function () {
    ...
    $('#externalStoreIDPOS').val(externalStoreID) // TODO: Agrego para que sea mas facil
    ...
    $.post('api/store/create/', { json: JSON.stringify(storeJSON) }, function (
      results
    ) {
  ...
  $('#createPOS').click(function () {
    ...
    $('#external_id').val(externalPOSID) // TODO: Agrego para que sea mas facil
    ...
    var posJSON = {
      name: posName,
      fixed_amount: true, // fixed amount debe ser true para no permitir cambios en el importe
      category: category,
      external_store_id: externalStoreID,
      external_id: externalPOSID,
    }

    $.post('api/pos/create/', { json: JSON.stringify(posJSON) }, function (
      results
    ) {
...
var items = [
  ...
  },
  {
    ...
    picture_url:
      'https://globalassets.starbucks.com/assets/c636153c255049a487da5db5b9d5f631.jpg?impolicy=1by1_wide_1242',
    description:
